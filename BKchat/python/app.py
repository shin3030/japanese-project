...
def get_prompt(prompt_type, content=""):
    if prompt_type == 1:
        ...
        return f"「{content}」請自行判斷並解析以上日文句子中使用的單字、文法。例句不需要中文翻譯。例句不需要例句解析。只能用中文回答。不要重複例句內容。若為多個句子需個別解析各個句子。嚴格遵守以下範例格式不要添加任何額外的文字或解析或這句話的意思:「單字解析：<br>- 單字1(若單字為漢字則加上日文平假名，反之若單字為平假名或片假名則什麼都不加，包括漢字、平假名、片假名和中文翻譯。)：單字1詳細解析<br>- 單字2(若單字為漢字則加上日文平假名，反之若單字為平假名或片假名則什麼都不加，包括漢字、平假名、片假名和中文翻譯。)：單字2詳細解析。<br>- 單字3(若單字為漢字則加上日文平假名，反之若單字為平假名或片假名則什麼都不加，包括漢字、平假名、片假名和中文翻譯。)：單字3詳細解析。<br><br>文法解析：<br>- 文法1：文法1解析<br>- 文法2：文法2解析<br>- 文法3：文法3解析<br>。」。嚴格遵守以上範例格式不要添加任何額外的文字或解析或這句話的意思"
